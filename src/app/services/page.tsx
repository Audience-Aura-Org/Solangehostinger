...
export default function ServicesPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#080808] flex items-center justify-center text-[9px] uppercase tracking-widest text-[#404040] animate-pulse">
        Loading Catalog...
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#080808] text-[#FDFBF7] pt-24 lg:pt-32 pb-24">

      {/* ─── Header ──────────────────────────────────────────────────── */}
      <section className="px-6 lg:px-12 max-w-7xl mx-auto mb-16 border-b border-[#141414] pb-14 text-center md:text-left">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <h1 className="text-5xl md:text-7xl lg:text-[6rem] font-serif leading-none tracking-tight">
            Our<br className="hidden md:block" /><span className="italic font-light text-[#C5A059] md:ml-0 ml-2">Services.</span>
          </h1>
          <div className="max-w-sm mx-auto md:mx-0">
            <p className="text-sm text-[#C8C0B0] leading-relaxed font-light mb-6">
              Six categories. Every size. One standard — absolute precision. Browse the full catalogue below.
            </p>
            <Link href="/booking"
              className="inline-block text-[9px] uppercase tracking-[0.3em] text-[#C5A059] border-b border-[#C5A059]/40 pb-1 hover:text-[#FDFBF7] hover:border-[#FDFBF7]/40 transition-all">
              Book a Session →
            </Link>
          </div>
        </div>
      ...
      <section className="px-6 lg:px-12 max-w-7xl mx-auto space-y-px">
        {services.map((svc) => {
          ...
          return (
            <div key={svc.id} className="border border-[#141414] bg-[#080808]">

              {/* ── Accordion Trigger ── */}
              <button
                className="w-full text-left px-6 py-6 flex items-center justify-between gap-4 group hover:bg-[#0C0C0C] transition-colors"
                onClick={() => setOpen(isOpen ? null : svc.id)}
              >
                <div className="flex items-center gap-6 flex-1 min-w-0">
                  <span className="text-[9px] font-mono text-[#404040] shrink-0">{svc.n}</span>
                  <h2 className={`text-xl md:text-2xl font-serif leading-none transition-colors ${isOpen ? 'text-[#C5A059]' : 'text-[#FDFBF7] group-hover:text-[#C5A059]'}`}>
                    {svc.title}
                  </h2>
                  <span className="text-[7px] uppercase tracking-[0.25em] text-[#C5A059] border border-[#C5A059]/25 px-2 py-0.5 hidden sm:block shrink-0">
                    {svc.tag}
                  </span>
                </div>

                <div className="flex items-center gap-8 shrink-0">
                  <span className="text-sm text-[#C5A059] tabular-nums hidden sm:block">
                    {svc.sizes[svc.sizes.length - 1].price}
                  </span>
                  <span className={`text-lg text-[#C5A059] transition-transform duration-300 ${isOpen ? 'rotate-45' : ''}`}>+</span>
                </div>
              </button>

              {/* ── Expanded Panel ── */}
              {isOpen && (
                <div className="border-t border-[#141414] grid md:grid-cols-[1fr_320px] gap-px bg-[#141414]">

                  {/* Left: Info */}
                  <div className="bg-[#080808] p-6 lg:p-8 space-y-8">

                    <div>
                      <h3 className="text-base font-serif text-[#C5A059] italic mb-3 leading-snug max-w-xl">
                        &ldquo;{svc.headline}&rdquo;
                      </h3>
                      <p className="text-sm text-[#C8C0B0] leading-relaxed max-w-2xl font-light">
                        {svc.description}
                      </p>
                    </div>
                    ...
                    {/* Benefits */}
                    <div>
                      <p className="text-[8px] uppercase tracking-[0.35em] text-[#5A5248] mb-4">What&apos;s Included</p>
                      <div className="grid sm:grid-cols-2 gap-x-8 gap-y-2">
                        {svc.benefits.map((b: string) => (
                          <div key={b} className="flex items-start gap-3">
                            <span className="text-[#C5A059] text-[10px] mt-0.5 shrink-0">—</span>
                            <span className="text-xs text-[#C8C0B0]">{b}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* Longevity + Care */}
                    <div className="grid sm:grid-cols-2 gap-8 border-t border-[#141414] pt-6">
                      <div>
                        <p className="text-[8px] uppercase tracking-[0.35em] text-[#5A5248] mb-3">Wear Life</p>
                        <p className="text-sm text-[#FDFBF7] font-serif">{svc.longevity}</p>
                      </div>
                      <div>
                        <p className="text-[8px] uppercase tracking-[0.35em] text-[#5A5248] mb-3">Care Guide</p>
                        <ul className="space-y-1.5">
                          {svc.care.map((c: string) => (
                            <li key={c} className="text-[10px] text-[#8A8070] leading-relaxed">· {c}</li>
                          ))}
                        </ul>
                      </div>
                    </div>
                  ...
                  {/* Right: Pricing Table */}
                  <div className="bg-[#060606] p-6 lg:p-8">
                    <p className="text-[8px] uppercase tracking-[0.35em] text-[#5A5248] mb-5">Pricing</p>
                    <div className="space-y-px">
                      {svc.sizes.map((sz: any, i: number) => (
                        <div
                          key={sz.label}
                          className={`flex items-center justify-between py-4 px-4 transition-colors ${i === 0 ? 'bg-[#C5A059]/5 border border-[#C5A059]/15' : 'bg-[#080808] border border-[#141414]'}`}
                        >
                          <div>
                            <p className="text-xs text-[#FDFBF7]">{sz.label}</p>
                            <p className="text-[9px] text-[#8A8070] mt-0.5">{sz.duration}</p>
                          </div>
                          <span className={`text-sm tabular-nums ${i === 0 ? 'text-[#C5A059]' : 'text-[#FDFBF7]'}`}>{sz.price}</span>
                        </div>
                      ))}
                    </div>

                    <p className="text-[8px] text-[#5A5248] mt-4 leading-relaxed">
      ...
      {/* ─── Bespoke CTA ─────────────────────────────────────────────── */}
      <section className="px-6 lg:px-12 max-w-7xl mx-auto mt-20 border border-[#141414] bg-[#060606]">
        <div className="py-12 lg:py-16 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div className="mx-auto md:mx-0">
            <span className="text-[8px] uppercase tracking-[0.4em] text-[#C5A059] block mb-3">Bespoke Services</span>
            <h2 className="text-3xl font-serif text-[#FDFBF7] leading-tight max-w-md mx-auto md:mx-0">
              Looking for something outside the catalogue?
            </h2>
            <p className="text-xs text-[#8A8070] mt-4 max-w-md leading-relaxed mx-auto md:mx-0">
          ...
          <div className="flex flex-col sm:flex-row gap-4 shrink-0 justify-center">
            <Link href="/contact"
              className="border border-[#222] text-[#C8C0B0] hover:border-[#C5A059]/40 hover:text-[#C5A059] py-3 px-6 text-[9px] uppercase tracking-[0.3em] transition-all text-center">
              Request Quote
            </Link>
            <Link href="/booking"
              className="bg-[#C5A059] text-[#080808] hover:bg-[#DFBE82] py-3 px-6 text-[9px] uppercase tracking-[0.3em] font-semibold transition-all text-center">
              Book Now
            </Link>
          </div>
        </div>
